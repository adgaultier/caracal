...
/// This crate has a runtime dependency on artifacts produced by the `stealth-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `stealth-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `stealth-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `stealth-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "stealth-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `stealth-ebpf`, so cargo will automatically rebuild us
        // if `stealth-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `stealth-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("stealth-ebpf");
